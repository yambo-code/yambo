...
AC_ARG_WITH(fft_libs,AS_HELP_STRING([--with-fft-libs=<libs>],
            [Link to FFT libraries],[32]),[],[])
AC_ARG_WITH(fft_path,AS_HELP_STRING([--with-fft-path=<path>],
            [Path to the FFT install directory],[32]),[],[])
AC_ARG_WITH(fft_libdir,AS_HELP_STRING([--with-fft-libdir=<path>],
            [Path to the FFT lib directory],[32]),[],[])
AC_ARG_WITH(fft_includedir,AS_HELP_STRING([--with-fft-includedir=<path>],
            [Path to the FFT include directory],[32]),[],[])
#
AC_ARG_ENABLE(internal_fftqe,AS_HELP_STRING([--enable-internal-fftqe],
            [Use internal QE FFT library]),[],[enable_internal_fftqe=no])
AC_ARG_ENABLE(internal_fftsg,AS_HELP_STRING([--enable-internal-fftsg],
            [Use internal Goedecker FFT library]),[],[enable_internal_fftsg=no])
AC_ARG_ENABLE(3d_fft,AS_HELP_STRING([--enable-3d-fft],[Use 3D FFT]),[],[])
#
AC_ARG_WITH(fftsg_fac, AS_HELP_STRING([--with-fftsg-fac=<val>],
