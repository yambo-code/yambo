...
#
AC_ARG_WITH(slepc_libs,AS_HELP_STRING([--with-slepc-libs=<libs>],
            [Use Slepc libraries <libs>],[32]))
AC_ARG_WITH(slepc_incs,AS_HELP_STRING([--with-slepc-incs=<incs>],
            [Use Slepc includes <incs>],[32]))
AC_ARG_WITH(slepc_path, AS_HELP_STRING([--with-slepc-path=<path>],
            [Path to the Slepc install directory],[32]),[],[])
AC_ARG_WITH(slepc_libdir,AS_HELP_STRING([--with-slepc-libdir=<path>],
            [Path to the Slepc lib directory],[32]))
AC_ARG_WITH(slepc_includedir,AS_HELP_STRING([--with-slepc-includedir=<path>],
            [Path to the Slepc include directory],[32]))
#
AC_ARG_WITH(petsc_libs,AS_HELP_STRING([--with-petsc-libs=<libs>],
            [Use Petsc libraries <libs>],[32]))
AC_ARG_WITH(petsc_incs,AS_HELP_STRING([--with-petsc-incs=<incs>],
            [Use Petsc includes <incs>],[32]))
AC_ARG_WITH(petsc_path, AS_HELP_STRING([--with-petsc-path=<path>],
            [Path to the Petsc install directory],[32]),[],[])
AC_ARG_WITH(petsc_libdir,AS_HELP_STRING([--with-petsc-libdir=<path>],
            [Path to the Petsc lib directory],[32]))
AC_ARG_WITH(petsc_includedir,AS_HELP_STRING([--with-petsc-includedir=<path>],
