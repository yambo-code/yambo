...
define clean_driver
 ...
 if [ "$(1)" = "int-libs"  ] ||                  [ "$(1)" = "all" ] ; then \
   EXTS="\.f90 \.o \.lock \.mk \.mod \.save \.tmp_source to_remove";WDIR="$(libdir)";TARG="$(INT_LIBS)";$(clean_dir_driver); \
 ...
 if [ "$(1)" = "driver"    ] || [ -z "$(1)" ] || [ "$(1)" = "all" ] ; then \
  EXTS="\.f90 \.o \.lock \.mk \.mod \.save \.tmp_source to_remove";WDIR="$(compdir)";TARG="driver";$(clean_dir_driver);\
 fi
 if [ "$(1)" = "Ydriver"   ] ||                  [ "$(1)" = "all" ] ; then \
   EXTS="\.f90 \.o \.lock \.mk \.mod \.save \.tmp_source to_remove";WDIR="$(libdir)/yambo/Ydriver/src";TARG="$(YLIBDRIVER)";$(clean_dir_driver);\
...
define clean_src_driver
 if [ "$(1)" = "src" ] || [ "$(1)" = "ypp" ] || [ "$(1)" = "interfaces" ] ; then \
  if  test -f config/stamps_and_lists/active_directories.list; then \
   TARG="";MSG="$(1)";EXTS="\.f90 \.o \.lock \.mk \.mod \.save \.tmp_source to_remove";WDIR="$(compdir)";\
