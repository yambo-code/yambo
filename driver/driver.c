...
#endif
#include "codever.h"
#if defined self  || PJ_RAS || PJ_REELS || PJ_PH || PJ_GKS || PJ_CYL_CUT || PJ_SPIN || PJ_SC || PJ_NO_HERM
 #include "self_cpp.h"
#endif
...
#if defined self  || PJ_RAS || PJ_REELS || spp || PJ_PH || PJ_GKS || PJ_CYL_CUT || PJ_SPIN || PJ_SC || SPP_PJ_PH || SPP_PJ_RAS || PJ_NO_HERM
 if (iif == 1 && ttd>0 ) 
 {
  system(edit_line);
  exit (0);
