...
*/
#define DEFINE_BKS_COMMON_INDEXES \
...
#define FILL_BSK_COMMON_INDEXES \
     ...
     ! NEWLINE \
     ! k and p indexes NEWLINE \
     ! NEWLINE \
     i_k_bz  = BS_T_grp(i_Tgrp_k)%table(i_Tk,1) ! k NEWLINE \
     i_p_bz  = BS_T_grp(i_Tgrp_p)%table(i_Tp,1) ! p = k - q_W NEWLINE \
     ! NEWLINE \
     ...
     ! NEWLINE \
     ! k-q and p-q indexes NEWLINE \
     ! NEWLINE \
     i_kmq_bz=qindx_X(iq,i_k_bz,1) ! k-q NEWLINE \
     i_pmq_bz=qindx_X(iq,i_p_bz,1) ! p-q = k - q_W -q NEWLINE \
     ! NEWLINE \
     !ig_kmq=qindx_X(iq,i_k_bz,2) NEWLINE \
     !ig_pmq=qindx_X(iq,i_p_bz,2) NEWLINE \
     !ig_kp_mq=G_m_G(ig_kmq,ig_pmq) NEWLINE \
     ! NEWLINE \
     ...
     ! NEWLINE \
     ! Bands and spin indexes NEWLINE \
     ! NEWLINE \
...
#define FILL_BSK_KERNEL_INDEXES \
     ! NEWLINE \
     ig_kmq    = qindx_X(iq,i_k_bz,2) ! Go shift to reach the first BZ NEWLINE \
     ig_pmq    = qindx_X(iq,i_p_bz,2) NEWLINE \
     ! NEWLINE \
     iq_W_bz_mq=qindx_B(i_kmq_bz,i_pmq_bz,1) !q_W_mq_bz+G_W_mq=kmq_bz-pmq_bz NEWLINE \
