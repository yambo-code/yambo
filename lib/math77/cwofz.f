      subroutine CWOFZ (Z, W, FLAG)
c Copyright (c) 1996 California Institute of Technology, Pasadena, CA.
c ALL RIGHTS RESERVED.
c Based on Government Sponsored Research NAS7-03001.
c>>2002-11-06 CWOFZ Krogh Corrected comments.
c>>1996-03-30 CWOFZ Krogh Added external stmt., removed INT in type st.
c>>1992-10-13 CWOFZ WVS Improve efficiency and avoid underflow.
c>>1992-03-13 CWOFZ FTK Removed implicit statements.
c>>1991-08-23 CWOFZ WV Snyder Initial adaptation to Math77
c
...
c     region.
c     Use R1MACH to fetch machine characteristics.
...
c
      ...
      real             C, H, H2, LN2, QLAMDA, QRHO, R1MACH, RSQPD2, RX
      real             RY, SX, SY, TX, TY, U, U1, U2, UV, V, V1, V2
      real             W(2), W1, X, XABS, XQUAD, XSUM, Y, YABS, YQUAD
      real             YSUM, Z(2)
      parameter (RSQPD2 =  1.128379167095512573896158903121545171688e0)
      parameter (LN2    = 0.6931471805599453094172321214581765680755e0)
      real             INOGXM, LGOV2, LGUND, MXGONI, SQLGOV
      save INOGXM, LGOV2, LGUND, MXGONI, SQLGOV
      data LGOV2 /-1.0e0/
c
      if (lgov2 .le. 0.0e0) then
        lgov2 = log(r1mach(2))
        sqlgov = sqrt(lgov2)
        lgov2 = lgov2 - ln2
        lgund = log(r1mach(1))
        mxgoni = sqrt(8.0e0 / r1mach(4)) / rsqpd2
        inogxm = 1.0e0 / mxgoni
      end if
c
      xabs = abs(z(1))
...
c
      if (x .gt. y) then
        qrho = x * sqrt(1.0e0 + (y/x)**2)
      else if (y .eq. 0.0e0) then
        qrho = 0.0e0
      else
        qrho = y * sqrt(1.0e0 + (x/y)**2)
      end if
c
      a = qrho .lt. 0.292e0
      if (a) then
...
c       cause any trouble here, because qrho .lt. 1.
        ...
        yquad = 2.0e0*xabs*yabs
        n     = int(6.5e0 + 72.0e0 * (1.0e0-0.85e0 * y) * qrho)
        j     = 2*n+1
        xsum  = rsqpd2/j
        ysum  = 0.0e0
        do 10 i = n, 1, -1
...
c         x*x + y*y is more accurate than qrho*qrho here:
          c    = (1.0e0-y) * sqrt(1.0e0-x*x-y*y)
          h    = 1.88e0 * c
          h2   = 2.0e0 * h
          nu   = int(17.5e0 + 26.0e0*c)
          kapn = int(8.5e0  + 34.0e0*c)
c         Select kapn so qlamda doesn't underflow.  Small kapn is good
c         (when possible) for performance also.
          if (h2 .lt. 0.25e0) kapn=min(kapn,1+int(lgund/log(h2)))
          qlamda = h2**(kapn-1)
c         0 < qlamda < 3.76**41 < 3.85e23.
          do 20 n = nu, kapn+1, -1
            tx  = yabs + h + n*rx
            ty  = xabs - n*ry
...
c         required accuracy.
          nu   = int(4.5e0 + (1442.0e0 / (26.0e0 * qrho + 77.0e0)))
          do 40 n = nu, 1, -1
            tx  = yabs + n*rx
            ty  = xabs - n*ry
...
c             The following protects 2*exp(-z**2) against overflow.
              ...
              w1 =  2.0e0*exp((yabs-xabs)*(yabs+xabs))
              uv = min(abs(u),abs(v))
              if (w1 .gt. uv) then
...
110   flag = +1
     ...
     1'Too few significant digits in COS(IMAG(Z**2)),',',')
      ...
      call serv1 ('SQRT(2*pi/eps)',mxgoni,',')
120   call serv1 ('REAL(Z)',z(1),',')
      call serv1 ('IMAG(Z)',z(2),'.')
      w(1) = r1mach(2)
      w(2) = w(1)
      return
c
