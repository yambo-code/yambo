*DECK DBESJ1
      DOUBLE PRECISION FUNCTION DBESJ1_ (X)
...
C
      ...
      IF (Y .LE. XMIN) CALL XERMSG ('SLATEC', 'DBESJ1_',
     +   'ABS(X) SO SMALL J1 UNDERFLOWS', 1, 1)
      IF (Y.GT.XMIN) DBESJ1_ = 0.5D0*X
      IF (Y.GT.XSML) DBESJ1_ = X*(.25D0 + DCSEVL (.125D0*Y*Y-1.D0,
     1  BJ1CS, NTJ1) )
      RETURN
C
 20   CALL D9B1MP (Y, AMPL, THETA)
      DBESJ1_ = SIGN (AMPL, X) * COS(THETA)
C
      RETURN
      END
