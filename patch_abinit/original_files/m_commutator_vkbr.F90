...
!arrays
  integer,pointer :: sign_dyad(:,:)   SET2NULL
  ! sign_dyad(lnmax,ntypat). 
  ! sign of the KB dyadic product.

  real(dp),pointer :: ff(:,:,:)  SET2NULL 
  ! ff(npw,lnmax,ntypat) 
  ! KB form factor.

  real(dp),pointer :: ffd(:,:,:)  SET2NULL
...
type,public :: kb_potential
 ...
 complex(gwpc),pointer :: gradvnl(:,:,:)   SET2NULL
 ! gradvnl(3,npwwfn,npwwfn)
 ! (grad_K + grad_K') Vnl(K,K') in reciprocal lattice units. used if inclvkb==0 (OBSOLETE)

 complex(gwpc),pointer :: fnl(:,:,:)     SET2NULL
 ! fnl(npwwfn,mpsang*mpsang,natom)

 !% complex(gwpc),pointer :: fnlr(:,:,:)     SET2NULL
 ! fnlr(nfft,mpsang*mpsang,natom)
 ! Real space representation of fnl.

 complex(gwpc),pointer :: fnld(:,:,:,:)  SET2NULL
 ! fnld(3,npwwfn,mpsang*mpsang,natom)

end type kb_potential
!!**

 interface nullify_kb_potential
   module procedure nullify_kb_potential_0D
   module procedure nullify_kb_potential_1D
 end interface nullify_kb_potential
...
!!****f* m_commutator_vkbr/nullify_kb_ffactors
!! NAME
!!  nullify_kb_ffactors
!!
!! FUNCTION
!!  Set all pointer of a structure of type kb_form_factors to NULL.
!!
!! PARENTS
!!
!! CHILDREN
...
subroutine nullify_kb_ffactors(KBff_k)


!This section has been created automatically by the script Abilint (TD).
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'nullify_kb_ffactors'
!End of the abilint section
...
!integer 
 nullify(KBff_k%sign_dyad)

!real
 nullify(KBff_k%ff )
 nullify(KBff_k%ffd)

end subroutine nullify_kb_ffactors
!!***
...
!!****f* m_commutator_vkbr/nullify_kb_potential_0D
!! NAME
!!  nullify_kb_potential_0D
!!
!! FUNCTION
!!  Set all pointers in a structure of type kb_potential to NULL.
!!
!! SIDE EFFECTS
!!  Kb<kb_potential>=All pointers are initialized to NULL.
!!
!! PARENTS
!!      m_commutator_vkbr
!!
...
subroutine nullify_kb_potential_0D(Kb)
...
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'nullify_kb_potential_0D'
!End of the abilint section
...
!Arguments ------------------------------------
!scalars
 type(kb_potential),intent(inout) :: Kb
...
end subroutine nullify_kb_potential_0D
!!***

!----------------------------------------------------------------------

!!****f* m_commutator_vkbr/nullify_kb_potential_1D
!! NAME
!!  nullify_kb_potential_1D
!!
!! FUNCTION
!!  Set all pointers in a structure of type kb_potential to NULL.
!!
...
subroutine nullify_kb_potential_1D(Kb)
...
!Do not modify the following lines by hand.
#undef ABI_FUNC
#define ABI_FUNC 'nullify_kb_potential_1D'
!End of the abilint section
...
!************************************************************************
 ...
 do ii=1,SIZE(Kb)
   call nullify_kb_potential_0D(Kb(ii))
 end do

end subroutine nullify_kb_potential_1D
!!***
...
!!
!! INPUTS
!!  Cryst<crystal_structure>=Datatype gathering info on the crystal structure.
!!  Psps<pseudopotential_type>Structure gathering info on the pseudopotentials.
...
!!
!! PARENTS
!!      calc_optical_mels,calc_sig_ppm_eet,cchi0q0,cchi0q0_intraband,m_shexc
!!
!! CHILDREN
!!      add_vnlr_commutator,metric,mkffnl,mkkin
!!
