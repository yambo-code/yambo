...
The argument longopts must be an array of these structures, one for each long
option. Terminate the array with an element containing all zeros.
...
- int has_arg
  This field says whether the option takes an argument. It is an integer, and
there are three legitimate values: no_argument, required_argument and
optional_argument.
...
which is the flag for this option. The value in val is the value to store in the
flag to indicate that the option was seen.
                                      ...
                                      int *use_mpi, int n_options)
{
    ...
    */
    ...
    for (i_opt = 0; i_opt < n_options; i_opt++)
    {
        ...
        */
        ...
        /* VARS */
        n_vars =
            opts[i_opt].int_var + opts[i_opt].float_var + opts[i_opt].char_var;
        if (n_vars == 0)
        {
    ...
    */
    ...
    int long_index = 0;
    while ((opt = getopt_long_only(argc, argv, opt_string, long_options,
                                   &long_index)) != -1)
    {
        /* No option valid */
        if (opt == '?')
        ...
        for (i_opt = 0; i_opt < n_options; i_opt++)
        {
            ...
            if (opts[i_opt].short_opt == opt)
            {
                break;
            };
        }
        /* DEBUG
        if (opt > 0) {printf ("GETOPT ouput: %c
        %s",opts[i_opt].short_opt,opts[i_opt].long_opt);} if (optarg != NULL)
        ...
        */
        ...
        if (strcmp(opts[i_opt].long_opt, "help") == 0)
        {
            ...
            */
            ...
            else
            {
                if (optarg == NULL)
                {
                    sprintf(dummy, "%s", "help");
                }
                if (optarg != NULL)
                {
                    sprintf(dummy, "%s", optarg);
                }
            }
            /* DEBUG
            printf ("USAGE call %s \n",dummy);
        ...
        if (strcmp(opts[i_opt].long_opt, "Input") == 0)
        {
            y.in_file = optarg;
            continue;
        }
        if (strcmp(opts[i_opt].long_opt, "Job") == 0)
        {
            y.job = optarg;
            continue;
        }
        if (strcmp(opts[i_opt].long_opt, "Idir") == 0)
        {
            y.in_dir = optarg;
            continue;
        }
        if (strcmp(opts[i_opt].long_opt, "Cdir") == 0)
        {
            y.com_dir = optarg;
            continue;
        }
        if (strcmp(opts[i_opt].long_opt, "parenv") == 0)
        {
            y.parenv_file = optarg;
            continue;
        }
        if (strcmp(opts[i_opt].long_opt, "Odir") == 0)
        {
            y.out_dir = optarg;
            continue;
        }
        if (strcmp(opts[i_opt].long_opt, "nompi") == 0)
        {
            *use_mpi = -1;
            continue;
        }
        if (strcmp(opts[i_opt].long_opt, "Quiet") == 0)
        {
            *use_editor = -2;
            continue;
        }
