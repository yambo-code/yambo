...
void options_control(struct options_struct *options, int *i_opt)
{
    ...
    *i_opt = *i_opt + 1;
    ...
    strcpy(options[*i_opt].long_desc[0],
           "<string> can be RL,kpt,sc,qp,io,gen,resp/X,ph,rt,par,nl,all");
    ...
    *i_opt = *i_opt + 1;
    ...
    strcpy(options[*i_opt].long_desc[0],
           "Format is DdHhMm with D=days, H=hours and M=minutes");
    ...
    *i_opt = *i_opt + 1;
    ...
    strcpy(options[*i_opt].long_desc[0],
           "The value is per processor. The value can be provided in Mb/Gb. "
           "E.g. 1Gb");
