...
void options_maker(struct options_struct *options, int n_options)
{
    ...
    for (i_opt = 0; i_opt < n_options; i_opt++)
    {
        ...
        {
            strcpy(options[i_opt].long_desc[i], "undef");
        }
    ...
    */
    ...
    for (i_opt = 0; i_opt < n_options; i_opt++)
    {
        if (options[i_opt].short_opt > 0)
        {
            continue;
        }
        if (options[i_opt].short_desc == NULL)
        {
            break;
        }
        j++;
        ...
        /* DEBUG
        printf("Short descripton %c assigned to long var
