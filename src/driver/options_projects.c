...
void options_projects(struct options_struct *options, int *i_opt)
{
    ...
    *i_opt = *i_opt + 1;
    options[*i_opt].short_desc =
        "Self-Consistent Static Electric Field Calculations";
    ...
    *i_opt=*i_opt+1;
    ...
    options[*i_opt].bin="yambo_ph";
    options[*i_opt].yambo_string="excph";
    options[*i_opt].section=desc;
    ...
    *i_opt = *i_opt + 1;
    ...
    strcpy(options[*i_opt].long_desc[0],
           "<string>=(p)ump or probe,(pp)ump & probe, (pn) n external fields");
    options[*i_opt].char_var = 1;
    ...
    *i_opt = *i_opt + 1;
    ...
    strcpy(options[*i_opt].long_desc[0],
           "<string>=(p)ump or probe,(n) non-linear optics");
    ...
    *i_opt = *i_opt + 1;
    ...
    strcpy(options[*i_opt].long_desc[i_desc],
           "<string>=(ee):electron-electron interaction");
#if defined _QED
    i_desc = i_desc + 1;
    strcpy(options[*i_opt].long_desc[i_desc],
           "<string>=(eh):electron-photon interaction");
#endif
#if defined _ELPH
    i_desc = i_desc + 1;
    strcpy(options[*i_opt].long_desc[i_desc],
           "<string>=(ep):electron-phonon interaction");
#endif
#if defined _ELPH
    i_desc = i_desc + 1;
    strcpy(options[*i_opt].long_desc[i_desc],
           "<string>=(pe):phonon-electron interaction");
#endif
    ...
    i_desc = i_desc + 1;
    strcpy(options[*i_opt].long_desc[i_desc],
           "Use -scattering ee+ep/ee+pe to activate more than one kind "
           "simultaneously");
#endif
