...
void usage(options_struct *options, struct tool_struct t, char *what,
           int n_options)
{
    ...
    */
    ...
    if (strcmp(what, "help") == 0)
    {
        ...
        {
            if (use_me(options, t, i_opt) == 0)
            {
                continue;
            }
            n_blanks = options[i_opt].int_var * 6 +
                       options[i_opt].float_var * 7 +
                       options[i_opt].char_var * 9;
            if (n_blanks > n_strings)
            {
                n_strings = n_blanks;
            }
        };
        ...
        for (i_o = 0; i_o < n_orders; i_o++)
        {
            ...
            for (i_opt = 0; i_opt < n_options; i_opt++)
            {
                ...
                if (strcmp(options[i_opt].section, order[i_o]) == 0)
                {
                    n_order_elements++;
                }
            };
            if (n_order_elements == 0)
            {
                continue;
            }
            fprintf(stderr, "\n %s:\n", order[i_o]);
            for (i_opt = 0; i_opt < n_options; i_opt++)
            {
                ...
                for (i = 1; i <= options[i_opt].int_var; i++)
                {
                    fprintf(stderr, " %s", "<int>");
                };
                for (i = 1; i <= options[i_opt].float_var; i++)
                {
                    fprintf(stderr, " %s", "<real>");
                };
                for (i = 1; i <= options[i_opt].char_var; i++)
                {
                    fprintf(stderr, " %s", "<string>");
                };
                n_blanks = n_strings + 2 - options[i_opt].int_var * 6 -
                           options[i_opt].float_var * 7 -
                           options[i_opt].char_var * 9;
                for (i = 1; i <= n_blanks; i++)
                {
                ...
                {
                    fprintf(stderr, " %s%s%s", "(more with -h ",
    ...
    else
    {
        for (i_opt = 0; i_opt < n_options; i_opt++)
        {
            ...
            if (strcmp(options[i_opt].long_opt, what) == 0)
            {
                break;
            }
        }
        ...
        if (options[i_opt].short_opt > 57)
        {
            fprintf(stderr, " Short option: %c\n", options[i_opt].short_opt);
        }
        if (options[i_opt].int_var + options[i_opt].float_var +
                options[i_opt].char_var >
            0)
        {
        ...
        if (strcmp(options[i_opt].long_desc[0], "undef") != 0)
        {
            ...
            {
                if (strcmp(options[i_opt].long_desc[i], "undef") == 0)
                {
                    continue;
                }
                fprintf(stderr, "\n              %s",
                        options[i_opt].long_desc[i]);
            }
        }
        fprintf(stderr, "\n\n");
        fprintf(stderr, "%s\n\n",
