        ...
        if (*s)
        {
            ...
            { /* we have a block */
                *s = ' ';
                ...
                if (getsym(s) != NULL)
                { /* error */
                    /*fprintf(stderr, "%s \"%s\" %s", "Block", s, "already
                     * defined");*/
                    do
                    { /* skip block */
                        c = parse_get_line(f, &s, &length);
                ...
                { /* parse block */
                    ...
                    do
                    {
                        ...
                        if (*s && *s != '%')
                        {
                            ...
                            l = rec->value.block->n;
                            rec->value.block->n++;
                            rec->value.block->lines = (sym_block_line *)realloc(
                                (void *)rec->value.block->lines,
                                sizeof(sym_block_line) * (l + 1));
                            rec->value.block->lines[l].n = 0;
                            rec->value.block->lines[l].fields = NULL;

                            /* parse columns */
                            for (s1 = s; (tok = strtok(s1, "|")) != NULL;
                                 s1 = NULL)
                            {
                                ...
                                rec->value.block->lines[l]
                                    .fields = (char **)realloc(
                                    (void *)rec->value.block->lines[l].fields,
                                    sizeof(char *) * (col + 1));
